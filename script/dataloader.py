...
class DatasetBag_addbag(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        majority_prop = torch.zeros(self.ins_label.max()+1)
        majority_prop[major_label] = (sum(ins_label==major_label)/len(ins_label))
        
        ins_label = torch.tensor(ins_label).long()
        major_label = torch.tensor(major_label).long()
        return {"bags": data, "ins_label": ins_label, "bag_label": major_label, "bag_idx":idx, "majority_prop": majority_prop}
